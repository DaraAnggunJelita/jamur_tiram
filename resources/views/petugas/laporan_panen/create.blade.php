            ...
            <div class="bg-[#FBF8F1] shadow-xs rounded-2xl border border-[#C9B896]/40 p-8">
                ...
                <div class="flex items-center space-x-2.5 pb-4 mb-6 border-b border-[#C9B896]/20">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-[#8E6E4E] font-medium">Catat berat dan kondisi panen jamur tiram hari ini.</p>
                    </div>
                </div>
                ...
                <form action="{{ route('petugas.laporan-panen.store') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select id="kondisi" name="kondisi"
                            ...
                            <option value="Bagus" {{ old('kondisi') === 'Bagus' ? 'selected' : '' }}>🟢 Bagus — Segar & Berkualitas</option>
                            <option value="Cukup" {{ old('kondisi') === 'Cukup' ? 'selected' : '' }}>🟡 Cukup — Sedikit Layu / Kecil</option>
                            <option value="Rusak" {{ old('kondisi') === 'Rusak' ? 'selected' : '' }}>🔴 Rusak — Layu / Busuk</option>
                        </select>
                        @error('kondisi')
                            <p class="text-[#A0653D] text-xs font-bold mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="catatan" name="catatan" rows="4"
                            ...
                            class="block w-full rounded-xl border-[#C9B896]/60 bg-white shadow-2xs focus:border-[#4F6146] focus:ring-[#4F6146] text-sm py-2.5 text-[#362C24] font-medium placeholder-[#8E6E4E]/50 @error('catatan') border-[#A0653D] @enderror">{{ old('catatan') }}</textarea>
                        @error('catatan')
                            <p class="text-[#A0653D] text-xs font-bold mt-1">{{ $message }}</p>
                        @enderror
                    </div>

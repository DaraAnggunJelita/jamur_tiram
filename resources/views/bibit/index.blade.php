<x-app-layout>
 ...
 <form method="GET" action="{{ route('bibit.index') }}" class="flex flex-col sm:flex-row items-center gap-4 mb-6">
 <div class="w-full sm:w-1/2">
 <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari berdasarkan kode, asal bibit..." class="w-full rounded-xl border-[#E5E7EB] text-sm focus:border-[#059669] focus:ring-[#059669]" oninput="clearTimeout(this.delay); this.delay = setTimeout(() => this.form.submit(), 500);">
 </div>
 ...
 <a href="{{ route('bibit.index') }}" class="p-2.5 bg-gray-500 text-white rounded-xl hover:bg-gray-600 transition shadow-md" title="Reset">
 ...
 <td class="py-3.5 px-4 font-bold text-[#064E3B] text-xs">{{ \Carbon\Carbon::parse($bibit->tanggal_masuk)->format('d M Y') }}</td>
 ...
 <a href="{{ route('bibit.edit', $bibit->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 bg-[#10B981] hover:bg-[#059669] text-white text-[10px] font-bold rounded-lg transition duration-150 shadow-xs cursor-pointer">
 Edit
 </a>

            ...
            <div class="bg-[#FBF8F1] rounded-2xl border border-[#C9B896]/40 p-6 sm:p-8 shadow-xs">
                ...
                <div class="flex items-center space-x-2.5 pb-4 mb-6 border-b border-[#C9B896]/20">
                    <div class="w-8 h-8 bg-[#7C9169]/15 rounded-lg flex items-center justify-center text-[#4F6146] text-lg">
                        <svg class='w-6 h-6 inline-block' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z'/></svg>
                    </div>
                    <div>
                        <h3 class="text-base font-black text-[#26201B] font-heading">Perbarui Jadwal Estimasi</h3>
                        <p class="text-xs text-[#8E6E4E] font-medium">Ubah perkiraan tanggal pemetikan jamur tiram.</p>
                    </div>
                </div>

                <form method="POST" action="{{ route('jadwal-panen.update', $jadwal->id) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="date" id="tanggal_estimasi" name="tanggal_estimasi"
                               value="{{ old('tanggal_estimasi', $jadwal->tanggal_estimasi) }}" required
                    ...
                    <div>
                        ...
                        <textarea id="catatan" name="catatan" rows="4"
                                  ...
                                  class="block w-full rounded-xl border-[#C9B896]/60 bg-white shadow-2xs focus:border-[#4F6146] focus:ring-0 text-sm py-2.5 text-[#26201B] placeholder-[#C9B896]/70 @error('catatan') border-[#A0653D] focus:border-[#A0653D] @enderror">{{ old('catatan', $jadwal->catatan) }}</textarea>
                        @error('catatan')
                            <p class="text-[#A0653D] text-xs font-bold mt-1 font-sans">{{ $message }}</p>
                        @enderror
                    </div>

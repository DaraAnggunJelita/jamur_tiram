        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-[#26201B] md:ml-2">Edit Produk</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="bg-[#FBF8F1] shadow-xs rounded-2xl border border-[#C9B896]/40 p-8">
                <h3 class="text-base font-black text-[#26201B] pb-4 mb-6 border-b border-[#C9B896]/20 flex items-center space-x-2 font-heading tracking-tight">
                    <svg class="w-5 h-5 text-[#6B4E36]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2.828 2.828 0 114 4L11.828 15H8v-3.828l9.282-9.282z"/></svg>
                    <span>Form Edit Produk</span>
                </h3>

                <form action="{{ route('ketua.catalogs.update', $catalog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 font-sans">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                            class="block w-full rounded-xl border-[#C9B896]/60 bg-white shadow-2xs focus:border-[#4F6146] focus:ring-[#4F6146] text-sm py-2.5 text-[#362C24] font-medium" required>{{ old('description', $catalog->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <div class="relative rounded-xl shadow-2xs">
                            ...
                            <input type="number" name="price" value="{{ old('price', $catalog->price) }}" min="0"
                                class="block w-full pl-9 rounded-xl border-[#C9B896]/60 bg-white focus:border-[#4F6146] focus:ring-[#4F6146] text-sm py-2.5 text-[#362C24] font-black font-mono-data" required>
                        </div>
                    ...
                    <div>
                        <label class="block text-xs font-black text-[#6B4E36] uppercase tracking-widest font-heading mb-1.5">Gambar Produk <span class="text-[#8E6E4E]/70 font-medium lowercase">(Kosongkan jika tidak diganti)</span></label>

                        @if($catalog->image)
                            <div class="mb-4 flex items-center space-x-3 bg-[#F6F1E6]/50 border border-[#C9B896]/40 p-3 rounded-xl w-fit shadow-2xs">
                                <img src="{{ asset('storage/'.$catalog->image) }}" alt="{{ $catalog->name }}" class="w-16 h-16 object-cover rounded-lg border border-[#C9B896]/40 bg-white">
                                <div>
                                    <p class="text-[10px] text-[#6B4E36] font-black uppercase tracking-wider font-heading">Gambar Saat Ini</p>
                                    <p class="text-xs text-[#8E6E4E] font-medium truncate max-w-[150px]">{{ basename($catalog->image) }}</p>
                                </div>
                            </div>
                        @endif
